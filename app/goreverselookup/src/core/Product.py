...
class Product:
    ...
    def fetch_ortholog(self, human_ortholog_finder: Optional[HumanOrthologFinder] = None, uniprot_api: Optional[UniProtApi] = None, ensembl_api: Optional[EnsemblApi] = None, goaf: Optional[GOAnnotationsFile] = None, prefer_goaf = False, _d_compare_goaf = False, model_settings:Optional[ModelSettings] = None) -> None:
        """
    ...
    async def fetch_ortholog_async(self, session: aiohttp.ClientSession, goaf: GOAnnotationsFile, human_ortholog_finder: Optional[HumanOrthologFinder] = None, uniprot_api: Optional[UniProtApi] = None, ensembl_api: Optional[EnsemblApi] = None, model_settings:Optional[ModelSettings] = None) -> None:
        """
        Fetches the ortholog for this product. If the ortholog query was successful, then self.genename is updated to the correct human ortholog gene name.
        Additionally, during the course of fetch_ortholog, ensembl_api.get_info may be called - if this happens, then the values description, ensg_id, enst_id, refseq_nt_id, uniprot_id are
        also filled out for this Product from the ensembl_api.get_info return value.
    ...
    async def fetch_ortholog_async_semaphore(self, session: aiohttp.ClientSession, semaphore:asyncio.Semaphore, goaf: GOAnnotationsFile, human_ortholog_finder: Optional[HumanOrthologFinder] = None, uniprot_api: Optional[UniProtApi] = None, ensembl_api: Optional[EnsemblApi] = None) -> None:
