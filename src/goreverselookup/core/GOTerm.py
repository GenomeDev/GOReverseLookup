...
class GOTerm:
    def __init__(
        ...
        """
        self.id = id
        self.processes = processes if isinstance(processes, set) else {processes}
        self.name = name
    ...
    async def fetch_name_description_async(
        self, api: GOApi, session: aiohttp.ClientSession, req_delay=0.1
    ):
    ...
    async def fetch_products_async_v3(
        ...
        if model_settings.target_organism is None:
            raise Exception(
                "Target organism was not specified in input.txt. Make sure to specify a target organism in the 'settings' section of input.txt!"
            )
        if request_params is not None:
            if "rows" in request_params:
                if request_params["rows"] < 10000000:
                    logger.warning(
                        f"MAJOR WARNING: Rows specified in request params ({request_params['rows']}) are less than {10000000}. You risk missing out important anootations!"
                    )
            if "rows" not in request_params:
                logger.warning(
                    f"MAJOR WARNING! You did not specify 'rows' in request params. You risk missing out important annotations!"
                )
        ...
        previous_data_taxa_dict = Cacher.get_data(
            "go", f"{data_key}_products-taxa-dict"
        )
        ...
        for _ in range(max_retries):
            ...
            if retries == (max_retries - 1):
                ...
                ModelStats.goterm_product_query_results[
                    self.id
                ] = f"Error: Exceeded max retries. Error info: {possible_http_error_text}"
                return []
            retries += 1
            ...
            if assoc["object"]["id"] == self.id:
                for database, taxa in approved_dbs_and_taxa.items():
                    if database in assoc["subject"]["id"] and any(
                        taxon in assoc["subject"]["taxon"]["id"] for taxon in taxa
                    ):
        ...
        logger.info(
            f"Fetched {len(self.products)} products for GO term {self.id} from {len(_d_unique_dbs)} unique databases ({_d_unique_dbs})"
        )
        Cacher.store_data("go", data_key, products)
        Cacher.store_data(
            "go", f"{data_key}_products-taxa-dict", self.products_taxa_dict
        )
