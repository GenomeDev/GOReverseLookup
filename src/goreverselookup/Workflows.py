...
from .core.Metrics import Metrics, adv_product_score, nterms, fisher_exact_test, binomial_test, basic_mirna_score
...
class Workflow:
    def __init__(self, input_file_fpath: str, save_folder_dir: str, model: ReverseLookup = None, name: str = ""):
        """
    ...
    def perform_scoring(self, scoring_classes: Metrics | list[Metrics], delete_previous:bool = True, recalculate:bool = True):
        """
    ...
    def generate_report(self, product_scoring_algorithm: Metrics = None, miRNA_scoring_algorithm: Metrics = None, report_filepath:str = "", verbosity:int = 3):
        """
        ...
        """
        def pick_scoring_algorithm(scoring_algorithm_class: Metrics, miRNA: bool = False):
            """
            ...
            if scoring_algorithm_class is not None:
                ...
                for key,value in self.computed_scores.items():
                    if miRNA == True and isinstance(key, basic_mirna_score):
                        return value
                    elif miRNA == False:
                        return value
                return None
        ...
        final_product_scoring_algorithm = pick_scoring_algorithm(product_scoring_algorithm)
        final_miRNA_scoring_algorithm = pick_scoring_algorithm(miRNA_scoring_algorithm, miRNA=True)
        
        # generate the report
        report_generator = ReportGenerator(self.model, verbosity=verbosity)
        report_generator.general_report(final_report_filepath, final_product_scoring_algorithm, final_miRNA_scoring_algorithm)
...
class WorkflowOne(Workflow):
    ...
    def create_worklfow(self, debug:bool = False):
        ...
        self.add_function(self.perform_scoring, scoring_classes=[adv_product_score, nterms], recalculate=False)
        self.add_function(self.model.save_model, self.model_save_filepath)
