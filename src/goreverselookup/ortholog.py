...
async def ensemblOrthologs(
    ...
    for source_id in source_ids:
        symbol = source_id.split(':', 1)[1] if len(source_id.split(':', 1)) > 1 else source_id
    ...
    async with aiohttp.ClientSession() as session:
        ...
        tasks = [
            _semaphore_get_request(url, session, semaphore)
    ...
    for source_id, result in zip(source_ids, results):
        ...
        homologies_list_of_dicts = result.get('data', [{}])[0].get('homologies') # is result["data"][0]["homologies"]
        if not homologies_list_of_dicts:
            pass

        best_ortholog = max(homologies_list_of_dicts, key=lambda a: a["target"]["perc_id"])

        target_ids[source_id].append(best_ortholog["target"]["id"])
...
def find_orthologs(
    ...
    if database == "gOrth":
        ...
        if not source_taxon or not target_taxon:
            return {}
        target_ids_dict = asyncio.run(ensemblOrthologs(source_ids_list, source_taxon, target_taxon))
