...
from ..parse.OrthologParsers import ZFINHumanOrthologFinder, MGIHumanOrthologFinder, RGDHumanOrthologFinder, XenbaseHumanOrthologFinder
    ...
    async def get_info_async(self, id:str, session:aiohttp.ClientSession):
        ...
        try:
            ...
            if previous_response != None:
                ...
                Cacher.store_data("url", url, response_json)
                await asyncio.sleep(self.async_request_sleep_delay)
        except (requests.exceptions.RequestException, TimeoutError, asyncio.CancelledError, asyncio.exceptions.TimeoutError, aiohttp.ClientResponseError, aiohttp.ClientOSError, aiohttp.ClientPayloadError):
            # If the request fails, try the xrefs URL instead
            try:
                # TODO: 19.08.2023: the below link doesn't work for any other {species} in endpoint other than human. Ie.
                # zebrafish, xenbase, mgi, rgd don't work !!!
